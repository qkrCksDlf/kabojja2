...
class Flux_kv_edit_inf(only_Flux):
    ...
    def forward(self, inp, inp_target, mask: Tensor, opts):
        ...
        if opts.attn_scale != 0 and (~bool_mask).any():
            attention_scale = self.create_attention_scale(L + 512, info['mask_indices'], device=mask.device,
                                                          scale=opts.attn_scale)
        else:
            attention_scale = None
        info['attention_scale'] = attention_scale

        denoise_timesteps = get_schedule(opts.denoise_num_steps, inp["img"].shape[1],
        ...
        with torch.no_grad():
            ...
            z_fe, info = denoise_kv_inf(self.model, img=inp["img"], img_ids=inp['img_ids'],
                                        ...
                                        target_txt=inp_target['txt'], target_txt_ids=inp_target['txt_ids'],
                                        target_vec=inp_target['vec'],
                                        timesteps=denoise_timesteps, source_guidance=opts.inversion_guidance,
                                        target_guidance=opts.denoise_guidance,
                                        info=info)
...
class Flux_kv_edit(only_Flux):
    ...
    def inverse(self, inp, mask, opts):
        ...
        z0 = inp["img"].clone()
        info['inverse'] = True
        zt, info = denoise_kv(self.model, **inp, timesteps=denoise_timesteps, guidance=opts.inversion_guidance,
    ...
    def denoise(self, z0, zt, inp_target, mask: Tensor, opts, info):
        ...
        denoise_timesteps = get_schedule(opts.denoise_num_steps, inp_target["img"].shape[1],
                                         shift=(self.name != "flux-schnell"))
        denoise_timesteps = denoise_timesteps[opts.skip_step:]
        ...
        if opts.attn_scale != 0 and (~bool_mask).any():
            attention_scale = self.create_attention_scale(L + 512, mask_indices, device=mask.device,
                                                          scale=opts.attn_scale)
        else:
            attention_scale = None
        info['attention_scale'] = attention_scale
        ...
        info['inverse'] = False
        x, _ = denoise_kv(self.model, **inp_target, timesteps=denoise_timesteps, guidance=opts.denoise_guidance,
                          inverse=False, info=info)

        z0[:, mask_indices, ...] = z0[:, mask_indices, ...] * (1 - info['mask'][:, mask_indices, ...]) + x * info[
                                                                                                                 'mask'][
                                                                                                             :,
                                                                                                             mask_indices,
                                                                                                             ...]
