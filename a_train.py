...
class FluxEditor_kv_demo:
    ...
    def inverse(self, brush_canvas,
        ...
        img_tokens = self.zt_r.to(self.device)
        img_ids = inp2["img_ids"].to(self.device)
        txt = inp2["txt"].to(self.device)
        txt_ids = inp2["txt_ids"].to(self.device)
        vec = inp2["vec"].to(self.device)
        denoised_tokens = denoise(
    model=flux_model,
    img=img_tokens,
    img_ids=img_ids,
    txt=txt,
    ...
    vec=vec,
    timesteps=timesteps,
    guidance=opts.denoise_guidance
)
        latents = unpack(denoised_tokens.float(), height, width)  # use original height/width

# Move decoder to the latent's device and decode
        self.ae.decoder.to(latents.device)
        ...
        result_img.save("abc.png", format="PNG")
