...
class FluxEditor_kv_demo:
    ...
    def inverse(self, brush_canvas,
        ...
        timesteps = get_schedule(
            num_steps=28,
            image_seq_len=inp["img_ids"].shape[1],
            base_shift=0.5,
            max_shift=1.15,
            shift=True
        )
        ...
        img_tensor = decoded[0].detach().cpu()  # [3, H, W]
    ...
    def edit(self, brush_canvas,
        ...
        rgba_ref_image = np.concatenate([ref_image, 255 * np.ones_like(ref_mask_bin[..., None])], axis=-1).astype(
            np.uint8)
